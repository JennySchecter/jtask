                ...
                <div class="row" style="overflow-x: scroll">
                    <div class="col-sm-12">
                        <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
                            ...
                            <tbody>
                            <?php $i = 0; foreach ($fbaLists as $fba):?>
                                <tr role="row" class="<?= $i%2==1? 'odd':'even' ;?>">
                                    <td><?php
                                        $userModel = \backend\models\User::find()->where(['username'=>$fba['memberName']])->one();
                                        echo $userModel['isvip']==1 ? $fba['codeNum'] . '<span class="badge bg-red">vip运单</span>': $fba['codeNum'];
                                        ?></td>
                                    <td><?=$fba['orderNum']?></td>
                                    <td><?=$fba['expressNum']?></td>
                                    <td><?=$fba['memberName']?></td>
                                    <td><?php
                                        $channelParent = \backend\models\Channel::find()->where(['id'=>$fba['channelParentId']])->one();
                                        $channelChild = \backend\models\Channel::find()->where(['id'=>$fba['channelChildId']])->one();
                                        echo $channelParent['name'] . '-' . $channelChild['name'];
                                        ?></td>
                                    <td><?php
                                        $country = \backend\models\Country::find()->where(['id'=>$fba['countryId']])->one();
                                        echo $country['name'];
                                        ?></td>
                                    <td><?=$fba['timeCreate']? date('Y-m-d H:i:s',$fba['timeCreate']) : ''?></td>
                                    <td><?=$fba['timeIn']? date('Y-m-d H:i:s',$fba['timeIn']) : ''?></td>
                                    <td><?=$fba['timeOut']? date('Y-m-d H:i:s',$fba['timeOut']) : ''?></td>
                                    <td><?=$fba['weightInput']?></td>
                                    <td><?=$fba['weightOutput']?></td>
                                    <td><?=$fba['remark']?></td>
                                    <td><?=$fba['flyNo']?></td>
                                    <td><?php
                                        echo
                                        \backend\helpers\AdminFun::getStatusValue($fba['status']);
                                        ?></td>
                                    <td>
                                        <?=Html::a('<i class="fa fa-info"></i>查看',yii\helpers\Url::to(['/fba/view','id'=>$fba['id']]),['class' => 'btn btn-default btn-xs'])?>
                                        <?=Html::a('<i class="fa fa-download"></i>下载','#',[
                                                'id' => 'download',
                                                'data-toggle' => 'modal',
                                        ...
                                        <?=Html::a('<i class="fa fa-edit"></i>修改',yii\helpers\Url::to(['/fba/update','id'=>$fba['id']]),['class'=>'btn bg-purple btn-xs'])?>
                                        <?=Html::button('<i class="fa fa-trash-o"></i>删除',[
                                            'class' => 'btn btn-xs btn-danger fba-delete',
                                            'key' => $fba['id']
                                        ])?>
                                        <?=Html::a('<button class="btn btn-xs btn-success"><i  class="fa fa-fw fa-warning"></i>异常件调查</button>',yii\helpers\Url::to(['/survey-list/auto-create-by-fba','id'=>$fba['id']]))?>
                                    </td>
                                </tr>
                                <?php $i++;?>
                            <?php endforeach;?>
...
Modal::begin([
        'id' => 'download-modal',
        'header' => '<h4 class="modal-title">下载附件</h4>',
        'footer' => '<button type="button" class="btn btn-outline" data-dismiss="modal">关闭</button>',
        'options' => [
...
$requestDownload = \yii\helpers\Url::toRoute('download');
$requestDelete = \yii\helpers\Url::toRoute('delete');
$js = <<<JS
        $(document).on('click','#download',function() {
          $.get('{$requestDownload}',{'fid':$(this).attr('key')},function(data) {
            $('.modal-body').html(data);
          });   
        });
        $(document).on('click','.fba-delete',function() {
            var fid = $(this).attr('key');
           layer.confirm('您确定删除该fba运单？',{
               btn:['确定','取消']
           },function() {
               $.post('{$requestDelete}',{'fid':fid},function(data) {
