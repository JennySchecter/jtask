        ...
        <div class="box-body no-padding">
            <table class="table table-condensed table-bordered text-center">
                <tbody>
                    ...
                    <tr>
                        <td>订单编号：</td>
                        <td><?= $form->field($model, 'order_num')->label(false)->textInput(['readonly'=>true,'value'=>$waybillModel->orderNum]) ?></td>
                        <td>客户名称：</td>
                        <td><?= $form->field($model, 'member_name')->label(false)->textInput(['maxlength' => true,'readonly'=>true,'value'=>$waybillModel->memberName]) ?></td>
                        <td>调查渠道：</td>
                        <td><?= $form->field($model, 'dc_channel')->label(false)->textInput(['readonly'=>true,'value'=>$dc_channel]) ?></td>
                    </tr>
                    <tr aria-colspan="2">
                        <td>情况描述：</td>
                        <td colspan="5">
